describe('Post Detail and Comments E2E Tests', () => {
  ...
  beforeEach(() => {
    ...
    cy.visit('/articles');
    cy.get('.post-card').first().click(); // "Lire la suite"
  ...
  it('should display the post content correctly', () => {
    ...
    cy.get('.meta-author').should('not.be.empty');
  ...
  it('should allow adding a new comment and see it in the list', () => {
    ...
    cy.get('.comments-section').then($section => {
      const initialCount = $section.find('.comment-row').length;

      // 2. Taper et envoyer le commentaire
      cy.get('textarea[formControlName="contenu"]').type(commentText);
      cy.get('button.btn-icon-send').should('not.be.disabled').click();
      ...
      cy.get('.comment-row').should('have.length', initialCount + 1);
      cy.get('.comment-row').last().within(() => {
        cy.get('.comment-author').should('contain', testUser.pseudo);
        cy.get('.comment-bubble').should('contain', commentText);
      });
    });
  });
  ...
  it('should disable send button when comment is too long or empty', () => {
    ...
    cy.get('button.btn-icon-send').should('be.disabled');
